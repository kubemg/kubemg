...
**On-demand state (`pkg/api/resources.go` + `resources_inventory.go`)**: `/api/v1/clusters/:id/resources/*` reads live cluster state through `bastion.Proxy.Call`, which is the same impersonation, namespace enforcement and audit trail as a kubectl call — the UI gets no privileged shortcut. `resources.go` holds the plumbing (`callResource`/`decodeResource`/`fetch`, `resourceCluster`, `resourceNamespace`) plus namespaces, workloads, pods and logs; `resources_inventory.go` holds the rest of the inventory the Explore sidebar browses: `deployments`, `statefulsets`, `daemonsets`, `jobs`, `cronjobs`, `services`, `ingresses`, `httproutes`, `virtualservices`, `persistentvolumes`, `persistentvolumeclaims`, `storageclasses`, `configmaps`, `secrets`, `crds`, `nodes`. Responses are normalised server-side so the browser does not need to know fifteen Kubernetes list shapes. Three rules hold across all of them: a namespace-scoped grant is answered **from the grant** rather than by listing the cluster (so a scoped user cannot enumerate namespaces they were not given) and is refused outright on a cluster-scoped list by `requireClusterScope`, because a cluster-wide list reaches past the scope; an optional CRD (Gateway API, Istio) that answers 404 is reported as an **empty list marked `available: false`** with a reason rather than as an error, via `fetchOptional`, which also falls back across API versions; and **`configmaps`/`secrets` return keys only — no value ever enters a response**, so nothing lands in a browser cache because someone opened a list.
