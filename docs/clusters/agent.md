...
`ghcr.io/kubemg/kubemg-agent:0.8.1`), overridable at runtime from **Settings →
...
```bash
make agent-image AGENT_VERSION=0.8.1     # builds ghcr.io/kubemg/kubemg-agent:0.8.1 locally
make agent-image-check                    # proves the amd64+arm64 matrix builds
make agent-push AGENT_VERSION=0.8.1       # requires docker login; pushes both arches
```
