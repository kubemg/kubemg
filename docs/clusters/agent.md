...
`ghcr.io/kubemg/kubemg-agent:0.11.0`), overridable at runtime from **Settings →
...
```dotenv
KUBEMG_AGENT_IMAGE=registry.internal/kubemg/kubemg-agent:0.11.0
```
...
```bash
make agent-image AGENT_VERSION=0.11.0     # builds ghcr.io/kubemg/kubemg-agent:0.11.0 locally
make agent-image-check                    # proves the amd64+arm64 matrix builds
make agent-push AGENT_VERSION=0.11.0       # requires docker login; pushes both arches
```
