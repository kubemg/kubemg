...
This page is for whoever runs it: what it does, what it needs, and what its logs
mean. Building it, the wire protocol and the two copies of its manifests are in
the developer guide's [The agent module](../dev/agent.md).
...
reports on the tunnel, so a pod that cannot reach the
bastion shows as **not ready** in the cluster's own `kubectl get pods` as
well as in kubemg.

## Log lines worth watching

The agent logs structured JSON to stdout. Worth
...
- `"agent stopped"` with a non-empty `error` — the process is exiting. A
  cancelled context is a normal shutdown and is not logged as an error.
