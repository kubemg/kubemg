...
- [ ] Integrate VictoriaMetrics for minimal footprint metrics — **query path not started.** The datasource above is registered and checked but nothing reads from it yet. The Metrics API above answers "what is this using right now"; it is a two-minute sliding window with no history, so a series backend is still needed for anything over time
- [ ] Integrate VictoriaLogs/Promtail for minimal footprint logs — **query path not started.** Logs today are read live from the pod through the tunnel, so nothing survives a pod restart and nothing is searchable across pods
